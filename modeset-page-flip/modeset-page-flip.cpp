#define LOG_TAG "modeset-page-flip"
...
static struct buffer_object buf1 = {0};
static struct buffer_object buf2 = {0};
...
static void modeset_page_flip_handler(int fd, unsigned int sequence, unsigned int tv_sec, unsigned int tv_usec, void *user_data)
{
	...
	drmModePageFlip(fd, crtc_id, flag?buf1.fb_id:buf2.fb_id, DRM_MODE_PAGE_FLIP_EVENT, user_data);
...
int main()
{
    ...
    drmEventContext eventContext = {0};
	uint32_t conn_id = 0;
	uint32_t crtc_id = 0;
    int ret = 0;
	...
	eventContext.page_flip_handler = modeset_page_flip_handler;
	drmModePageFlip(fd, crtc_id, buf1.fb_id, DRM_MODE_PAGE_FLIP_EVENT, &crtc_id);
